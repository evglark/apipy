...
def login_user(
    ...
    for credentials in credentials_db:
        if credentials["user_id"] == user["id"] and verify_password(password, credentials["password_hash"]):
            ...
            _log_event(security_events_db, "login_success", user_id=user["id"], session_id=session_id)
            return {
...
def logout_user(refresh_token: str, refresh_tokens_db, token_blacklist, security_events_db):
    payload = decode_jwt(refresh_token)
    if not payload or payload.get("type") != "refresh":
...
def register_user(name: str, password: str, users_db, credentials_db, security_events_db):
    user = _find_user_by_name(name, users_db)
    if user:
